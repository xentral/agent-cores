...
- **Phase 1 — read-only ERP core slice (code complete, live-verification pending):**
  the adapter engine (`WeclappAdapterBase`) + the important ERP core entities, all
  read-only: **Customer, Supplier** (over `/party`), **Article**, **Quotation,
  SalesOrder** (with `orderItems`), **SalesInvoice, CreditNote, PurchaseOrder,
  Shipment**. list/read with filter/sort/pagination/counts, epoch-ms + FK +
  line-item + address translation, credentials-missing 424 — covered by engine unit
  tests. **Still required for v1:** reconcile weclapp property names against a live
  tenant's Swagger and run the effect-based checks per entity (see below).
