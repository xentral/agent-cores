...
and must never be derived from one another:
  ...
  * ``priorities.json``        — what it must be able to RECORD and FIND (domain expert)
...
def _field_rows(priorities: dict, model: dict, verified: dict) -> list[dict[str, Any]]:
    ...
    """
    rows: list[dict[str, Any]] = []
    entities = priorities.get("entities") or {}
...
def render_yaml(core_id: str, source: dict, summary, cap_rows, field_rows) -> str:
    ...
    header = (
        ...
        "# specification (capabilities.spec.yaml / priorities.json) or the code.\n"
...
def build(core_id: str) -> tuple[dict, list[dict], list[dict], list[dict]]:
    ...
    priorities_path = core_dir / "priorities.json"
    priorities = json.loads(priorities_path.read_text("utf-8")) if priorities_path.is_file() else {}
    ...
    field_rows = _field_rows(priorities, model, verified)
