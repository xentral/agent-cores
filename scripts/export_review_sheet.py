...
def _field_rows(core_id: str, field_gaps: dict, model: dict, verified: dict) -> list[dict[str, Any]]:
    ...
    """
    rows: list[dict[str, Any]] = []
    # The gaps left the specification: it describes what the system IS, and the parked
    # work list lives in backlog.yaml, which nothing loads at runtime.
...
def render_yaml(core_id: str, source: dict, summary, cap_rows, field_rows) -> str:
    ...
    header = (
        ...
        "# specification (erp-spec.yaml / backlog.yaml) or the code.\n"
...
def build(core_id: str) -> tuple[dict, list[dict], list[dict], list[dict]]:
    ...
    gaps_path = core_dir / "backlog.yaml"
    ...
    field_rows = _field_rows(core_id, field_gaps, model, verified)
