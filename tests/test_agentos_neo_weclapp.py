...
def test_roster_is_customer_only():
    assert set(ADAPTERS) == {"Customer"}
    assert ENTITY.endpoint == "party"
    assert CUSTOMER.manifest.operations == ("list", "read")
    assert CUSTOMER.manifest.adapter == "agentos_neo_weclapp.Customer"
